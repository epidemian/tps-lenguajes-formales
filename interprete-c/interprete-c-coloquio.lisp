;; Modificaciones para coloquio del intérprete de pseudo-C
...
(defun crear-vector (dim)
  (if (eq dim 0) nil (cons 0 (crear-vector (- dim 1)))))

(defun crear-matriz (filas cols)
  (mapcar (lambda (x) (crear-vector cols)) 
          (crear-vector filas)))

; Agrega una serie de variables a una memoria dada.
(defun agregar-variables (declaraciones mem)
  (if (null declaraciones) mem
      (cond
        ((eq (nth 1 declaraciones) '=)
         ; Declaración de una variable entera inicializada. 
         (cons (list (nth 0 declaraciones) (nth 2 declaraciones)) 
               (agregar-variables (cdddr declaraciones) mem)))
        ((eq (nth 1 declaraciones) '[)
         ; Declaración de una variable matriz.
         (cons (list (nth 0 declaraciones) (crear-matriz (nth 2 declaraciones) 
                                                         (nth 5 declaraciones)))
               (agregar-variables (cddddr (cdddr declaraciones)) mem)))
        (t 
         ; Declaración de variable entera sin inicialización. 
         (cons (list (nth 0 declaraciones) 0)
               (agregar-variables (cdr declaraciones) mem))))))

; Ejecuta un programa en pseudo-C
(defun run(prg ent &optional(mem nil))
  (if (null prg) nil
      (if (eq (caar prg) 'int) 
          (run (cdr prg) ent (agregar-variables (cdar prg) mem))
          (if (eq (caar prg) 'main) 
              (ejec (nth 1 (car prg)) ent mem ) 
              'no-hay-main))))

(defun asignar-matriz (var mem expresion  &optional (expr-fila nil))
  (if (eq (car expresion) '])
      (asignar-matriz-fila var mem (cddr expresion) expr-fila)
      (asignar-matriz var mem (cdr expresion) 
                      (append expr-fila (list (car expresion))))))

(defun asignar-matriz-fila (var mem expresion expr-fila &optional (expr-col))
  (if (eq (car expresion) '])
      (asignar-matriz-fila-col var mem (cdr expresion) expr-fila expr-col)
      (asignar-matriz-fila var mem (cdr expresion) expr-fila 
                           (append expr-col (list (car expresion))))))

(defun asignar-matriz-fila-col (var mem expr-valor expr-fila expr-col)
  (asignar var (reemplazar-valor-matriz (car expr-valor) 
                                        (valor-memoria var mem) (valor expr-fila mem)
                                        (valor expr-col mem)  
                                        (valor (cdr expr-valor) mem)) 
           mem))

(defun reemplazar-valor-matriz (op mat fila col val)
  (if (eq fila 0)
      (cons (reemplazar-valor-vec op (car mat) col val) (cdr mat))
      (cons (car mat) (reemplazar-valor-matriz op (cdr mat) (- fila 1) col val))))

(defun reemplazar-valor-vec (op vec pos val)
  (if (eq pos 0)
      (cons (if (eq op '=) val (funcall (buscar-operador op) (car vec) val)) (cdr vec))
      (cons (car vec) (reemplazar-valor-vec op (cdr vec) (- pos 1) val))))
        ...
        ((es-variable (caar prg) mem)
         (cond
           ...
           ((eq (nth 1 (car prg)) '[)
            ...
            (ejec (cdr prg) ent
                  (asignar-matriz (caar prg) mem (cddar prg))
                  sal))
           (t
            ; Transforma las sentencias a += b en a = a + b. Y a++ en a = a + 1
